...
eps_nuc1=eps_reac1
My_mass=67382.8987#8302.4127#1318.9874#3453.7680#433.5805#56058.4013#47151.8234#14.8086#12261.7898


avg_H1_frac1 = np.full(shape=(len(H1_frac1)-1), fill_value=np.nan)
delta_M_r1 = np.full(shape=(len(M_r1)-1), fill_value=np.nan)

myindex1 = np.where(M_r1-My_mass < 1.e-5)[0][0]
...
myindex2 = np.where(M_r2-My_mass < 1.e-5)[0][0]

...
for index, model in enumerate(models):
    ...
    X_H1 = np.sum(H1[idx_90:-1] * dM[idx_90:])
    X_C12 = np.sum(C12[idx_90:-1] * dM[idx_90:])
    X_N14 = np.sum(N14[idx_90:-1] * dM[idx_90:])
    X_O16 = np.sum(O16[idx_90:-1] * dM[idx_90:])
...
plt.title('Integrated mass ejected at 90% of each model')
plt.legend()

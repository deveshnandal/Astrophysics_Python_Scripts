...
H1_integral=[]
for k in range(max(Shell_loop)-2,min(Shell_loop),-1):
#    H1_integral[k] = H1_integral[k + 1] + avg_H1_frac[k] * delta_M_r[k] * (M_sun)
    H1_integral = np.append(H1_integral,avg_H1_frac[k] * delta_M_r[k] * (M_sun))

avg_eps_nuc = np.full(shape=(len(eps_nuc)-1), fill_value=np.nan)


for k in range(len(eps_nuc)-2):
